...
func validate(data any, db *DB) error {
	...
	logHandler.DatabaseLogger.Printf("[CON]{VALIDATE} Validate [%+v] [%v.db]", dao.GetStructType(data), db.Name)
	err := commonErrors.HandleGoValidatorError(dataValidator.Struct(data))
	if err != nil {
		logHandler.DatabaseLogger.Printf("[CON]{VALIDATE} error validating %v %v [%v.db]", err.Error(), dao.GetStructType(data), db.Name)
		timer.Stop(0)
